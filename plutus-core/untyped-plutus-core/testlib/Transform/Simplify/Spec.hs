...
interveningLambda =
  force (lam n (delay (var n `app` var n)) `app` con 1)
  where
    n = name "a"

caseOfCase1 :: Term Name PLC.DefaultUni PLC.DefaultFun ()
caseOfCase1 =
  case_ (ite (var (name "b")) true false) [con 1, con 2]
...
caseOfCase2 =
  case_ (ite (var b) (var t) false) [con 1, con 2]
  where
    (b, t) = freshNames2 "b" "t"
...
caseOfCase3 = case_ (ite (var b) trueBranch false) [altTrue, altFalse]
  where
    (b, x, xs, f) = freshNames4 "b" "x" "xs" "f"
...
floatDelay1 = lam a body `app` delay (con 1)
  where
    a = name "a"
    body = plus (force (var a)) (force (var a))
...
floatDelay2 = lam a body `app` delay arg
  where
    a = name "a"
    body = plus (force (var a)) (force (var a))
    arg = plus (con 1) (con 2)
...
floatDelay3 = lam a body `app` delay (con 1)
  where
    (a, b) = freshNames2 "a" "b"
    secondArg = force (lam b (var b) `app` var a)
    body = plus (force (var a)) secondArg

basicInline :: Term Name PLC.DefaultUni PLC.DefaultFun ()
basicInline = lam n (var n) `app` con 1
  where
    n = name "a"
...
mkInlinePurityTest termToInline = runQuote $ do
  ...
  let withTopLevelBindings = mkIterLamAbs (UVarDecl () <$> freeVars)
  pure $ withTopLevelBindings $ lam a (lam b (var a)) `app` term
...
inlinePure3 = mkInlinePurityTest $ do
  x <- freshName "x"
  y <- freshName "y"
  let t = app (lam x $ lam y $ var x `app` var x) (con 1)
      vars = []
  pure (vars, t)
...
inlineImpure1 = mkInlinePurityTest $ pure ([], err)

-- | @force (delay error)@ is impure.
inlineImpure2 :: Term Name PLC.DefaultUni PLC.DefaultFun ()
inlineImpure2 = mkInlinePurityTest $ pure ([], force . delay $ err)
...
inlineImpure3 =
  mkInlinePurityTest $ pure ([], force . force . force . delay . delay $ delay err)
...
inlineImpure4 = mkInlinePurityTest $ do
  a <- freshName "a"
  pure ([a], force . force . force . delay . delay $ var a)
...
callsiteInline = lam f $ lam g $ app fun arg
  where
    (a, f, g, x, y) = freshNames5 "a" "f" "g" "x" "y"
    fun =
      lam a $
        mkIterAppNoAnn
          (var f)
    ...
    arg = lam x . lam y $ mkIterAppNoAnn (var g) [var y, var x]
...
forceDelayMultiApply =
  ...
  where
    (x1, x2, x3, f, funcVar) = freshNames5 "x1" "x2" "x3" "f" "funcVar"
...
forceDelayComplex = whole
  where
    (x, y1, y2, y3, z1, z2, f, funcVar) = freshNames8 "x" "y1" "y2" "y3" "z1" "z2" "f" "funcVar"
    term =
      delay
        . delay
        . lam x
...
forceCaseDelayWithApps2 =
  lam scrut $ force $ case_ (var scrut) [lam x $ delay (con 1), delay (con 2)]
  where
    (scrut, x) = freshNames2 "scrut" "x"
...
forceCaseDelayWithApps2Fail =
  ...
  where
    (scrut, x, y) = freshNames3 "scrut" "x" "y"

-- case ((\x -> constr 0 []) 1) [42]
letFloatOutCase1 :: Term Name PLC.DefaultUni PLC.DefaultFun ()
letFloatOutCase1 = case_ (lam x true `app` con 1) [con 42]
  where
    x = name "x"
...
cse3 = lam f $ lam x $ var f `app` arg1 `app` arg2
  where
    (x, y, z, f) = freshNames4 "x" "y" "z" "f"
    arg1 = lam y (plus (con 1) (plus (var y) (var y))) `app` plus (con 0) (var x)
    arg2 = lam z (plus (con 2) (plus (var z) (var z))) `app` plus (con 0) (var x)
...
-- | @ite cond t f@ builds @ifThenElse cond t f@ with the type-force applied.
ite :: T -> T -> T -> T
ite cond t f = force (builtin PLC.IfThenElse) `app` cond `app` t `app` f
